...
export const appRouter = router({
  ...
  auth: router({
    ...
    updateMyOffice: protectedProcedure.input(z.object({ officeName: z.string().max(180) })).mutation(({ ctx, input }) => updateMyOffice(input.officeName, ctx.user)),
  }),
  procurement: router({
    dashboard: protectedProcedure.query(({ ctx }) => getProcurementDashboard(ctx.user)),
    analytics: router({
      endUserPerformance: protectedProcedure
        .input(
          z
            .object({
              source: z.enum(["all", "live", "historical"]).optional(),
    ...
    setup: router({
      ...
      updateSettings: protectedProcedure.input(z.object({ entityName: z.string().min(3).max(180), authorizedOfficialName: z.string().max(180).optional(), authorizedOfficialDesignation: z.string().max(160).optional(), chiefAccountantName: z.string().max(180).optional(), defaultNoticeSignatory: z.string().max(180).optional(), sessionTimeoutMinutes: z.number().int().min(5).max(240).optional(), enableInAppNotifications: z.boolean().optional(), notificationRefreshSeconds: z.number().int().min(10).max(120).optional(), appearanceTheme: z.enum(["light", "dark", "high_contrast"]).optional(), appearanceFont: z.enum(["public_sans", "system", "serif", "mono", "humanist"]).optional(), appearanceFontScale: z.enum(["90", "100", "110", "120"]).optional(), appearanceDensity: z.enum(["compact", "comfortable", "spacious"]).optional(), appearanceAccent: z.enum(["maroon", "teal", "blue", "forest"]).optional(), appearanceCorners: z.enum(["sharp", "soft", "round"]).optional(), appearanceReducedMotion: z.boolean().optional() })).mutation(({ ctx, input }) => { assertRole(normalizeProcurementRole(ctx.user.role), ["admin"]); return updateProcurementSettings(input, ctx.user); }),
    ...
    catalog: router({
      ...
      save: protectedProcedure.input(z.object({ items: z.array(z.object({ catalogItemId: z.number().int().positive(), quantity: z.number().positive().max(1000000) })).max(500) })).mutation(({ ctx, input }) => replaceProcurementCatalogSavedItems(input, ctx.user)),
