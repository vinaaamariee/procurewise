...
export const appRouter = router({
  ...
  procurement: router({
    ...
    setup: router({
      ...
      updateSettings: protectedProcedure.input(z.object({ entityName: z.string().min(3).max(180), authorizedOfficialName: z.string().max(180).optional(), authorizedOfficialDesignation: z.string().max(160).optional(), chiefAccountantName: z.string().max(180).optional(), defaultNoticeSignatory: z.string().max(180).optional(), sessionTimeoutMinutes: z.number().int().min(5).max(240).optional(), enableInAppNotifications: z.boolean().optional(), notificationRefreshSeconds: z.number().int().min(10).max(120).optional(), appearanceTheme: z.enum(["light", "dark", "high_contrast"]).optional(), appearanceFont: z.enum(["system", "serif", "mono", "humanist"]).optional(), appearanceFontScale: z.enum(["90", "100", "110", "120"]).optional(), appearanceDensity: z.enum(["compact", "comfortable", "spacious"]).optional(), appearanceAccent: z.enum(["maroon", "teal", "blue", "forest"]).optional(), appearanceCorners: z.enum(["sharp", "soft", "round"]).optional(), appearanceReducedMotion: z.boolean().optional() })).mutation(({ ctx, input }) => { assertRole(normalizeProcurementRole(ctx.user.role), ["admin"]); return updateProcurementSettings(input, ctx.user); }),
