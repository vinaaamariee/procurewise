...
const values = items.flatMap((item) => [item.source, item.productCode, item.description, item.unit, item.referencePrice, item.remarks, item.imageUrl, item.sourceAsOfDate, 1]);
...
try {
  ...
  const [rows] = await connection.query("SELECT COUNT(*) AS catalogCount, COUNT(imageUrl) AS imageCount FROM procurement_catalog_items WHERE source = ? AND isActive = 1", ["PhilGEPS common-use supplies and equipment"]);
