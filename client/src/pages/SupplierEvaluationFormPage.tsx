...
function DownloadPanel({ title, input }: { title: string; input: FormPdf }) { return <section className="mt-5 flex flex-wrap items-center justify-between gap-3 border border-[#d9c28f] bg-[#fffaf0] px-4 py-3"><p className="text-xs text-[#634b1d]">{title}. Download the supplied-form PDF for printing or authorized sharing.</p><Button type="button" size="sm" onClick={async () => await downloadSupplierEvaluationFormPdf(input)} className="h-8 rounded-[4px] bg-[#7b1e1e] text-[11px] hover:bg-[#641818]"><Download className="mr-1.5 h-3.5 w-3.5" />Download PDF</Button></section>; }
