...
export function SupplierEvaluationFormPage() {
  ...
  return <div className="mx-auto max-w-[1120px] py-1">
    {audience ? <form onSubmit={submit} className="border border-[#dfd9ce] bg-white dark:border-[#46515c] dark:bg-[#1b2229]"><OfficialFormHeader audience={audience} /><div className="p-5 sm:p-7"><div className="grid gap-x-8 gap-y-4 md:grid-cols-2">
      {audience === "end_user" ? <>
        ...
        <FormField label="Registration Date" value={selectedPhilgepsRegisteredAt ? new Date(selectedPhilgepsRegisteredAt).toLocaleDateString("en-PH") : "Not recorded in supplier registry"} />
        <FormField label="PhilGEPS Expiration Date" value={selectedPhilgepsExpiresAt ? new Date(selectedPhilgepsExpiresAt).toLocaleDateString("en-PH") : "Not recorded in supplier registry"} />
      </> : <>
    ...
    {audience === "end_user" && selectedSupplier && !selectedPhilgepsNumber && <div className="mt-5 border border-[#d9c28f] bg-[#fffaf0] p-3 text-[11px] leading-5 text-[#72561d] dark:border-[#635028] dark:bg-[#272118] dark:text-[#f0c36a]">PhilGEPS registration information is not recorded for this supplier. Procurement Staff should update the supplier registry before relying on this evaluation record.</div>}
    {isUrgentPrReference && <div className="mt-5 border border-[#d9c28f] bg-[#fffaf0] dark:border-[#635028] dark:bg-[#272118] p-3"><Label htmlFor="urgent-pr-reason" className="text-[11px] font-semibold text-[#634b1d] dark:text-[#f0c36a]">Urgent Purchase Request reference reason (system audit control)</Label><Textarea id="urgent-pr-reason" value={urgentPurchaseRequestReason} onChange={(event) => setUrgentPurchaseRequestReason(event.target.value)} className="mt-2 min-h-16 text-xs dark:bg-[#1b2229] dark:text-[#f1f5f8] dark:border-[#46515c]" maxLength={1000} placeholder="State why the displayed PR reference differs from the linked Purchase Request." /></div>}
    <OfficialInstructions /></div>
    <div className="mx-5 flex items-center gap-2 border-x border-t border-[#222] dark:border-[#64717d] bg-[#fffaf0] dark:bg-[#272118] px-3 py-2 text-[10px] text-[#72561d] dark:text-[#f0c36a] sm:hidden"><ArrowRight className="h-3.5 w-3.5 shrink-0" />Swipe the matrix horizontally to view and rate all response columns.</div>
    <div className="overflow-x-auto"><table className="min-w-[800px] w-full border-collapse text-left"><thead><tr className="bg-white dark:bg-[#2b3540] text-[10px] font-bold text-black dark:text-[#ffffff]"><th className="w-[48%] border border-[#222] dark:border-[#64717d] px-3 py-2 text-center text-[#1f2933] dark:text-[#ffffff]">CRITERIA</th>{SUPPLIER_EVALUATION_RATINGS.map((rating) => <th key={rating.score} className="w-[13%] border border-[#222] dark:border-[#64717d] px-2 py-2 text-center leading-3 text-[#1f2933] dark:text-[#ffffff]">{rating.label.toUpperCase()}<br />({rating.score})</th>)}</tr></thead><tbody>{criteria.map((group) => <GroupRows key={group.section} section={group.section} criteria={group.criteria} responses={responses} setResponses={setResponses} />)}<tr><td colSpan={5} className="border border-[#222] dark:border-[#64717d] px-3 py-2"><Label htmlFor="evaluation-remarks" className="text-[11px] italic text-[#52606d] dark:text-[#d1dae2]">Additional comments, suggestions, recommendations, and/or feedback.</Label><Textarea id="evaluation-remarks" value={remarks} onChange={(event) => setRemarks(event.target.value)} className="mt-2 min-h-20 border-0 p-0 text-xs shadow-none focus-visible:ring-0 text-[#1f2933] dark:text-[#f1f5f8] dark:bg-transparent" maxLength={3000} /></td></tr></tbody></table></div>
    <div className="flex flex-wrap items-end justify-between gap-5 px-5 pb-5 pt-6 sm:px-7"><div className="w-full max-w-md"><Label htmlFor="respondent-name" className="text-[11px] text-[#1f2933] dark:text-[#f1f5f8]">Name and Signature of Respondent:</Label><Input id="respondent-name" value={respondentName} onChange={(event) => setRespondentName(event.target.value)} className="mt-1 h-8 rounded-none border-x-0 border-t-0 border-b border-[#222] dark:border-[#64717d] dark:bg-[#1b2229] dark:text-[#f1f5f8] px-0 text-xs shadow-none" maxLength={180} /></div><div className="w-40"><Label className="text-[11px] text-[#1f2933] dark:text-[#f1f5f8]">Date:</Label><div className="mt-1 h-8 border-b border-[#222] dark:border-[#64717d] pt-1 text-xs text-[#1f2933] dark:text-[#f1f5f8]">{new Date().toLocaleDateString("en-PH")}</div></div><Button disabled={isSaving} className="h-9 rounded-[4px] bg-[#7b1e1e] text-xs text-white hover:bg-[#641818] dark:bg-[#d65c50] dark:text-white dark:hover:bg-[#eb766a] print:hidden no-print">{isSaving && <LoaderCircle className="mr-1.5 h-3.5 w-3.5 animate-spin text-white" />}Submit</Button></div>
    <OfficialFormFooter />
    </form> : <div className="border border-[#dfd9ce] bg-white dark:border-[#46515c] dark:bg-[#1b2229] p-8 text-center"><ShieldCheck className="mx-auto h-7 w-7 text-[#9a6d19] dark:text-[#f0c36a]" /><p className="mt-3 text-sm font-semibold text-[#1f2933] dark:text-[#f1f5f8]">Supplier Evaluation Form</p><p className="mt-1 text-[11px] text-[#77818d] dark:text-[#aeb9c4]">This controlled form is completed only by its designated End-User or Procurement Office respondent.</p></div>}
    {completedPdf && <div className="print:hidden no-print"><DownloadPanel title="Completed Supplier Evaluation Form saved" input={completedPdf} /></div>}
    {isApprover && <div className="print:hidden no-print"><ElectronicApprovalSection pending={pendingApprovals.data ?? []} selectedId={approvalEvaluationId} onSelectedIdChange={setApprovalEvaluationId} designation={approverDesignation} onDesignationChange={setApproverDesignation} consent={approvalConsent} onConsentChange={setApprovalConsent} isSaving={signApproval.isPending} approverName={user?.name || user?.email || "Authorized approver"} onSign={() => { if (!selectedPendingApproval) return toast.error("Select a completed Supplier Evaluation Form to approve."); if (!approverDesignation.trim()) return toast.error("Enter the authorized approver designation."); if (!approvalConsent) return toast.error("Confirm the electronic approval statement before signing."); signApproval.mutate({ supplierEvaluationId: selectedPendingApproval.evaluation.id, approverDesignation: approverDesignation.trim() }); }} /></div>}
    {signedPdf && <div className="print:hidden no-print"><DownloadPanel title="Electronic approval recorded" input={signedPdf} /></div>}
  </div>;
}

function OfficialFormHeader({ audience }: { audience: SupplierEvaluationAudience }) { return <header className="official-form-header print-include border-b-[3px] border-[#9a6d19] bg-white px-5 pb-4 pt-3 text-center dark:bg-white"><img src="/header.png" alt="Batanes State College official header" className="mx-auto h-auto w-full max-w-[760px] object-contain" /><p className="mt-2 text-[10px] font-semibold uppercase tracking-[0.16em] text-[#7b1e1e]">Procurement Unit</p><h1 className="mt-2 text-base font-bold text-[#202833]">SUPPLIER EVALUATION FORM</h1><p className="text-sm font-semibold text-[#1f2933]">(Goods)</p><p className="mt-1 text-xs italic text-[#52606d]">To be accomplished by {audience === "end_user" ? "end-user" : "Procurement Office"}</p></header>; }
function OfficialFormFooter() { return <footer className="official-form-footer print-include mt-8 border-t border-[#d8b04d] bg-white px-5 py-3 dark:bg-white"><img src="/footer.png" alt="Batanes State College official footer" className="mx-auto h-auto w-full max-w-[760px] object-contain" /></footer>; }
function OfficialInstructions() { return <section className="mt-7 text-[11px] leading-5 text-[#222] dark:text-[#d1dae2]"><p className="font-bold italic text-[#1f2933] dark:text-[#ffffff]">Instructions:</p><p className="mt-1">This is a survey on the performance of our suppliers. It aims to improve our procurement service/system. Your sincere and honest answers will be highly appreciated and treated with utmost confidentiality.</p><p className="mt-4">Please rate the supplier according to each criterion provided and put a checkmark (✓) on the column that best corresponds to your answer.</p><p className="mt-4">Thank you very much.</p></section>; }
function FormSelect({ label, value, onValueChange, placeholder, children }: { label: string; value: string; onValueChange: (value: string) => void; placeholder: string; children: React.ReactNode }) { return <div><Label className="text-[11px] font-medium text-[#1f2933] dark:text-[#f1f5f8]">{label} :</Label><Select value={value} onValueChange={onValueChange}><SelectTrigger className="mt-1 h-8 rounded-none border-x-0 border-t-0 border-b border-[#222] dark:border-[#64717d] dark:bg-[#1b2229] dark:text-[#f1f5f8] px-0 text-xs shadow-none"><SelectValue placeholder={placeholder} /></SelectTrigger><SelectContent className="dark:bg-[#1b2229] dark:border-[#46515c] dark:text-[#f1f5f8]">{children}</SelectContent></Select></div>; }
function FormField({ label, value, editable, onChange, type = "text" }: { label: string; value: string; editable?: boolean; onChange?: (value: string) => void; type?: "text" | "date" }) { return <div><Label className="text-[11px] font-medium text-[#1f2933] dark:text-[#f1f5f8]">{label} :</Label>{editable ? <Input type={type} value={value} onChange={(event) => onChange?.(event.target.value)} className="mt-1 h-8 rounded-none border-x-0 border-t-0 border-b border-[#222] dark:border-[#64717d] dark:bg-[#1b2229] dark:text-[#f1f5f8] px-0 text-xs shadow-none" /> : <div className="mt-1 h-8 border-b border-[#222] dark:border-[#64717d] pt-1 text-xs text-[#1f2933] dark:text-[#f1f5f8]">{value}</div>}</div>; }
function GroupRows({ section, criteria, responses, setResponses }: { section: string; criteria: Array<{ key: string; label: string }>; responses: Record<string, number>; setResponses: React.Dispatch<React.SetStateAction<Record<string, number>>> }) { return <><tr><th colSpan={5} className="border border-[#222] dark:border-[#64717d] bg-[#f1f3f5] dark:bg-[#232c35] px-3 py-1 text-left text-[10px] font-bold uppercase text-[#1f2933] dark:text-[#ffffff]">{section}</th></tr>{criteria.map((criterion) => <tr key={criterion.key}><td className="border border-[#222] dark:border-[#64717d] px-3 py-1.5 text-[11px] leading-4 text-[#1f2933] dark:text-[#f1f5f8]">{criterion.label}</td>{SUPPLIER_EVALUATION_RATINGS.map((rating) => <td key={rating.score} className="border border-[#222] dark:border-[#64717d] px-2 py-1 text-center"><input aria-label={`${criterion.label}: ${rating.label}`} type="radio" name={criterion.key} value={rating.score} checked={responses[criterion.key] === rating.score} onChange={() => setResponses((current) => ({ ...current, [criterion.key]: rating.score }))} className="h-3.5 w-3.5 accent-[#8f2424] dark:accent-[#d65c50]" /></td>)}</tr>)}</>; }
function DownloadPanel({ title, input }: { title: string; input: FormPdf }) { return <section className="mt-5 flex flex-wrap items-center justify-between gap-3 border border-[#d9c28f] bg-[#fffaf0] dark:border-[#635028] dark:bg-[#272118] px-4 py-3"><p className="text-xs text-[#634b1d] dark:text-[#f0c36a]">{title}. Download the supplied-form PDF for printing or authorized sharing.</p><Button type="button" size="sm" onClick={async () => await downloadSupplierEvaluationFormPdf(input)} className="h-8 rounded-[4px] bg-[#7b1e1e] text-[11px] text-white hover:bg-[#641818] dark:bg-[#d65c50] dark:text-white dark:hover:bg-[#eb766a]"><Download className="mr-1.5 h-3.5 w-3.5 text-white" />Download PDF</Button></section>; }
function ElectronicApprovalSection({ pending, selectedId, onSelectedIdChange, designation, onDesignationChange, consent, onConsentChange, isSaving, approverName, onSign }: { pending: Array<{ evaluation: { id: number; reportedPurchaseRequestNumber: string | null; evaluationAudience: string }; supplier: { companyName: string } | null; purchaseOrder: { poNumber: string } | null }>; selectedId: string; onSelectedIdChange: (value: string) => void; designation: string; onDesignationChange: (value: string) => void; consent: boolean; onConsentChange: (value: boolean) => void; isSaving: boolean; approverName: string; onSign: () => void }) { return <section className="mt-5 border border-[#d9c28f] bg-[#fffaf0] dark:border-[#635028] dark:bg-[#272118] p-5"><div className="flex items-start gap-3"><ShieldCheck className="mt-0.5 h-4 w-4 text-[#7b1e1e] dark:text-[#ff837a]" /><div><p className="text-sm font-semibold text-[#4d3711] dark:text-[#f0c36a]">Electronic approval signature</p><p className="mt-1 text-[11px] leading-5 text-[#705e35] dark:text-[#d1dae2]">This secure system control is recorded below—not inside—the controlled government form. It binds the authenticated approver, designation, consent, timestamp, and integrity reference to the completed evaluation.</p></div></div><div className="mt-4 grid gap-4 md:grid-cols-2"><FormSelect label="Completed Supplier Evaluation Form" value={selectedId} onValueChange={onSelectedIdChange} placeholder="Select completed form">{pending.map((entry) => <SelectItem key={entry.evaluation.id} value={String(entry.evaluation.id)}>{entry.purchaseOrder?.poNumber || `Evaluation #${entry.evaluation.id}`} — {entry.supplier?.companyName || "Recorded supplier"}</SelectItem>)}</FormSelect><FormField label="Authorized approver" value={approverName} /><FormField label="Designation" value={designation} editable onChange={onDesignationChange} /></div>{!pending.length && <p className="mt-4 text-xs text-[#705e35] dark:text-[#aeb9c4]">There are no completed Supplier Evaluation Forms awaiting electronic approval.</p>}<label className="mt-4 flex items-start gap-2 text-xs leading-5 text-[#4d3711] dark:text-[#d1dae2]"><Checkbox checked={consent} onCheckedChange={(checked) => onConsentChange(checked === true)} className="mt-0.5" />{consentStatement}</label><div className="mt-4 flex justify-end"><Button type="button" disabled={isSaving || !pending.length} onClick={onSign} className="h-9 rounded-[4px] bg-[#7b1e1e] text-xs text-white hover:bg-[#641818] dark:bg-[#d65c50] dark:text-white dark:hover:bg-[#eb766a]">{isSaving && <LoaderCircle className="mr-1.5 h-3.5 w-3.5 animate-spin text-white" />}Apply electronic signature</Button></div></section>; }
