...
export default function Dashboard() {
  ...
  const cards = [
    { label: "PPMP & Purchase Requests", icon: FileText, value: data?.purchaseRequests.length, detail: "End-User procurement packages visible to your role.", tone: "text-[#7b1e1e] bg-[#fff4f1]", href: "/purchase-requests" },
    { label: "Pre-Canvass packages", icon: FileCheck2, value: data?.preCanvasses.length, detail: "Three-supplier quote packages available for handoff.", tone: "text-[#325d91] bg-[#f1f6fc]", href: "/rfq" },
    { label: "POs, delivery & PMR", icon: CircleDollarSign, value: data?.purchaseOrders.length, detail: "Purchase Orders progressing through delivery and PMR.", tone: "text-[#9a6d19] bg-[#fff8e8]", href: "/purchase-orders" },
  ...
  return <div className="mx-auto max-w-[1240px]"><PageHeader eyebrow="Control center" title="Procurement overview" description="A role-sensitive view of PPMP, Purchase Request, Pre-Canvass, approval, Purchase Order, delivery, PMR, and accountability records." />
    ...
    <div className="mt-7 grid gap-6 xl:grid-cols-[1.4fr_0.8fr]">{data?.purchaseRequests.length ? <div className="flat-panel"><div className="border-b border-[#ece8df] px-5 py-4"><p className="text-sm font-semibold text-[#34404e]">Active Purchase Requests</p><p className="mt-1 text-[11px] text-[#7d8793]">Live records routed through the role-gated workflow.</p></div><div className="divide-y divide-[#f0ede6] px-5">{data.purchaseRequests.slice(0, 4).map((pr) => <div key={pr.id} className="flex items-center justify-between gap-3 py-4"><div><p className="text-xs font-semibold text-[#3e4855]">{pr.prNumber}</p><p className="mt-1 max-w-[480px] truncate text-[11px] text-[#77818d]">{pr.purpose}</p></div><StatusBadge tone={pr.status.includes("review") ? "pending" : pr.status === "approved" ? "approved" : "draft"}>{pr.status.replaceAll("_", " ").toUpperCase()}</StatusBadge></div>)}</div></div> : <EmptyWorkspace eyebrow="Action queue" title="There are no workflow actions awaiting your role." description="Requests and approvals requiring your authority will appear here as transactions are submitted and routed." actionLabel="Create purchase request" actionHref="/purchase-requests" />}
      <div className="flat-panel h-fit"><div className="flex items-center justify-between border-b border-[#ece8df] px-5 py-4"><div><p className="text-xs font-semibold text-[#34404e]">Process integrity</p><p className="mt-1 text-[11px] text-[#7d8793]">Mandatory controls for every transaction</p></div><Activity className="h-4 w-4 text-[#7b1e1e]" /></div><div className="divide-y divide-[#f0ede6] px-5 py-1">{["End-Users forward PPMP, PR, and three supplier Pre-Canvass quotes as one package.", "Procurement Officers create the Abstract of Canvass and lowest compliant recommendation.", "POs are issued after administrative approval, then closed only after delivery and PMR logging."].map((item, index) => <div key={item} className="flex gap-3 py-4"><StatusBadge tone={index === 1 ? "pending" : "approved"}>{index === 1 ? "REVIEW" : "CONTROL"}</StatusBadge><p className="text-[11px] leading-5 text-[#65707e]">{item}</p></div>)}</div></div>
    </div>
  </div>;
}
