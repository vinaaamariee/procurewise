...
function formTitle(doc: jsPDF, marker: string, title: string) {
  ...
  doc.setFontSize(12); doc.text("[AGENCY / INSTITUTION NAME]", 105, 15, { align: "center" }); doc.setFontSize(14); doc.text(title, 105, 22, { align: "center" });
}
...
export async function downloadPreCanvassPdf(input: { preCanvass: { preCanvassNumber: string; approvedBudget: string | null; quotationDeadline: Date | null; deliveryPeriodDays: number | null; priceEvaluationMode: string | null; createdAt: Date }; supplierName?: string | null; items: Array<{ description: string; quantity: string; unit: string }> }, options?: { preview?: boolean }) {
  ...
  invokeOfficialPdfDownload(doc, officialFormFileName("pre_canvass", input.preCanvass.preCanvassNumber), options?.preview);
}

export async function downloadRfqAcknowledgementPdf(input: { preCanvassNumber: string; suppliers: Array<{ companyName: string; receivedBy?: string | null; receivedAt?: Date | string | null }> }, options?: { preview?: boolean }) {
...
export async function downloadAbstractPdf(input: { abstract: { abstractNumber: string; openingDate: Date; openingLocation: string; procurementCategory: string; recommendationReason: string; status: string; recommendedSupplierId: number }; suppliers: string[]; items: Array<{ description: string }> }, options?: { preview?: boolean }) {
  const doc = new jsPDF({ unit: "mm", format: "a4" }); await addBscLetterhead(doc); doc.setFont("helvetica", "italic"); doc.setFontSize(7); doc.text("Annex F", 14, 9); doc.setFont("helvetica", "bold"); doc.setFontSize(11); doc.text("ABSTRACT OF QUOTATION", 105, 19, { align: "center" }); doc.setFont("helvetica", "normal"); doc.setFontSize(6.5); doc.text("(x) Furnishing/delivery of supplies, and materials or equipment", 34, 28); doc.text("( x ) Furnishing Labor, services, etc.", 34, 34); doc.text("(   ) Rental or use of transportation facilities equipment, quarters, rooms, lot or space, etc.", 34, 40); doc.text(`Bids opened at ${input.abstract.openingLocation || "Procurement Conference Room"}`, 196, 31, { align: "right" }); doc.text(date(input.abstract.openingDate) || "___________", 196, 37, { align: "right" }); doc.text("To be furnished at  [Agency / Institution Name]", 70, 47); doc.setFontSize(5.6); doc.text("(State place or site of Office or project where articles or services be furnished or returned)", 70, 52);
  ...
  invokeOfficialPdfDownload(doc, officialFormFileName("abstract", input.abstract.abstractNumber), options?.preview);
}

export async function downloadPpmpPdf(input: { fiscalYear: number; entries: Array<{ description: string; plannedAmount: string | number; actualAmount: string | number; status: string; officeName: string; objectOfExpenditureName: string; projectTitle?: string | null; papCode?: string | null; modeOfProcurement?: string | null; fundSource?: string | null; procurementSchedule?: string | null }> }, options?: { preview?: boolean }) {
  ...
  line(doc, 14, 31, 130, "Fiscal Year:", input.fiscalYear); line(doc, 153, 31, 130, "Institution:", "[Agency / Institution Name]");
...
export async function downloadAbstractPackagePdf(input: { abstract: { abstractNumber: string; openingDate: Date; openingLocation: string; procurementCategory: string; recommendationReason: string; status: string; recommendedSupplierId: number }; preCanvassNumber: string; quotes: Array<{ supplierId: number; totalPrice: string | number; deliveryDays: number; isCompliant: number; quotationReference?: string | null; acknowledgedAt?: Date | null; receivedBy?: string | null; notes?: string | null }>; supplierNames: Map<number, string>; items: Array<{ description: string; specification?: string | null; quantity: string | number; unit: string; estimatedUnitCost?: string | number }> }, options?: { preview?: boolean }) {
  ...
  y += 9; doc.setFont("helvetica", "bold"); doc.setFontSize(8); doc.text("CERTIFICATION / RECOMMENDATION", 14, y); doc.setFont("helvetica", "normal"); doc.setFontSize(7.2); doc.text(doc.splitTextToSize(input.abstract.recommendationReason, 260), 14, y + 7); signBlock(doc, Math.min(y + 28, 171), "Prepared by Procurement / BAC:", "Recommended / approved by:"); footer(doc); invokeOfficialPdfDownload(doc, `${input.abstract.abstractNumber}_AnnexF_AbstractPackage.pdf`, options?.preview);
}
...
export function downloadBestValuePolicyHistoryPdf(history: BestValuePolicyHistoryEntry[]) {
  ...
  const startPage = () => {
    ...
    doc.text("ProcureWise · Controlled Institutional Policy Record", 196, 31, { align: "right" });
  };
  startPage();
  let y = 40;
  if (!history.length) {
...
export async function downloadPurchaseOrderPdf(input: { purchaseOrder: { poNumber: string; totalAmount: string; status: string; placeOfDelivery: string | null; scheduledDeliveryDate: Date | null; deliveryTerm: string | null; paymentTerm: string | null; modeOfProcurement: string | null; fundCluster: string | null; orsBursNumber: string | null; fundsAvailable: string | null; authorizedOfficialName: string | null; authorizedOfficialDesignation: string | null; chiefAccountantName: string | null }; supplierName?: string; supplierTin?: string | null; items?: Array<{ description: string; stockPropertyNo?: string | null; unit: string; quantity: string; estimatedUnitCost: string; totalCost: string }> }, options?: { preview?: boolean }) {
  ...
  invokeOfficialPdfDownload(doc, officialFormFileName("purchase_order", input.purchaseOrder.poNumber), options?.preview);
}
