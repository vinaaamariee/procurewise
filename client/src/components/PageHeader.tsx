...
export function PageHeader({
  ...
  return (
    <div className="flex flex-col gap-4 border-b border-[#e4e1da] dark:border-[#46515c] pb-5 sm:flex-row sm:items-end sm:justify-between">
      <div>
        <p className="text-xs font-bold uppercase tracking-[0.14em] text-[#9a6d19] dark:text-[#f0c36a]">{eyebrow}</p>
        <h1 className="mt-1 font-display text-2xl font-semibold tracking-[-0.03em] text-[#202833] dark:text-[#f1f5f8] sm:text-[27px]">{title}</h1>
        <p className="mt-1.5 max-w-2xl text-[15px] leading-6 text-[#677281] dark:text-[#d1dae2]">{description}</p>
      </div>
      {action && (
