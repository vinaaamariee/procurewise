  ...
  return (
    <div className="grid min-h-[340px] place-items-center rounded-[6px] border border-dashed border-[#d9d5cd] bg-white px-6 text-center shadow-[0_1px_0_rgba(33,43,54,0.03)]">
      <div className="max-w-md">
        ...
        {actionLabel && (actionHref || actionOnClick) && (actionOnClick ? <Button type="button" onClick={actionOnClick} className="mt-6 h-9 rounded-[4px] bg-[#7b1e1e] px-4 text-xs font-semibold hover:bg-[#641818]">{actionLabel}<ArrowRight className="ml-2 h-3.5 w-3.5" /></Button> : <Button asChild className="mt-6 h-9 rounded-[4px] bg-[#7b1e1e] px-4 text-xs font-semibold hover:bg-[#641818]"><Link href={actionHref!}>{actionLabel}<ArrowRight className="ml-2 h-3.5 w-3.5" /></Link></Button>)}
      </div>
    </div>
  );
}
